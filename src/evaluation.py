...
class ModelEvaluator:
    ...
    def calculate_metrics(self, y_true, y_pred, y_proba: Optional[np.ndarray] = None) -> Dict[str, float]:
        """
        ...
        # Add per-class metrics if binary or multi-class
        if len(set(y_true)) == 2:
        ...
        if y_proba is not None:
            try:
                if len(set(y_true)) == 2:
                    metrics['roc_auc'] = roc_auc_score(y_true, y_proba[:, 1] if y_proba.shape[1] > 1 else y_proba.flatten())
                else:
                    # Multi-class ROC-AUC
...
class HumanQualityRater:
    ...
    def rate_response(self, original_tweet: str, predicted_intent: str, 
                     predicted_severity: int, generated_reply: str,
                     rating: int, rater_id: Optional[str] = None) -> Dict:
        """
        ...
        if not (config.HUMAN_RATING_SCALE[0] <= rating <= config.HUMAN_RATING_SCALE[1]):
            raise ValueError(f"Rating must be between {config.HUMAN_RATING_SCALE[0]} and {config.HUMAN_RATING_SCALE[1]}")
    ...
    def get_average_rating(self) -> float:
        ...
        if not self.ratings:
            return 0.0
        return np.mean([r['rating'] for r in self.ratings])
    
    def get_rating_distribution(self) -> Dict[int, int]:
        """Get distribution of ratings."""
        distribution = {}
        for rating in range(config.HUMAN_RATING_SCALE[0], config.HUMAN_RATING_SCALE[1] + 1):
            distribution[rating] = sum(1 for r in self.ratings if r['rating'] == rating)
        return distribution
...
def evaluate_all_models(y_true_intent, y_pred_intent, y_true_severity, y_pred_severity,
                        y_proba_intent=None, y_proba_severity=None) -> Tuple[Dict[str, float], Dict[str, float]]:
    """
    ...
    intent_metrics = intent_evaluator.calculate_metrics(y_true_intent, y_pred_intent, y_proba_intent)
    severity_metrics = severity_evaluator.calculate_metrics(y_true_severity, y_pred_severity, y_proba_severity)
    ...
    if intent_evaluator.check_accuracy_threshold(0.8):
        ...
        print(f"\n⚠ Intent classifier accuracy ({intent_evaluator.metrics['accuracy']:.2%}) below 80% requirement.")
