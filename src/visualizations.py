...
class VisualizationGenerator:
    ...
    def plot_confusion_matrix(self, y_true, y_pred, labels: Optional[List] = None,
    ...
    def plot_roc_curve(self, y_true, y_proba, labels: Optional[List] = None,
                      title: str = "ROC Curve",
                      task_type: str = "intent",
                      save: bool = True) -> None:
        """
        ...
        if n_classes == 2:
            ...
            fpr, tpr, _ = roc_curve(y_true, y_proba[:, 1] if y_proba.shape[1] > 1 else y_proba.flatten())
            roc_auc = auc(fpr, tpr)
            ...
            for i, color in zip(range(n_classes), colors):
                label_name = labels[i] if labels and i < len(labels) else f'Class {i}'
    ...
    def plot_error_analysis(self, y_true, y_pred, texts: Optional[List[str]] = None,
        ...
        ax1.set_title(f'Confusion Matrix - {task_type.upper()}', fontsize=12, fontweight='bold')
        ax1.set_ylabel('True Label', fontsize=10)
        ax1.set_xlabel('Predicted Label', fontsize=10)
        
        # Plot 2: Top misclassifications bar chart
        ...
        ax2.set_title(f'Top {top_n} Misclassifications', fontsize=12, fontweight='bold')
    ...
    def generate_results_table(self, model_results: Dict[str, Dict[str, float]],
    ...
    def plot_data_distribution(self, labeled_df: pd.DataFrame,
                              save: bool = True) -> None:
        """
        ...
        ax2.bar(severity_counts.index.astype(str), severity_counts.values, color='coral')
